WITH
dwh_delta AS ( -- Определение дельты изменений для заказчиков
    SELECT     
...
dwh_delta_insert_result AS ( -- Расчёт новых данных для вставки
    SELECT  
    ...
    FROM (
        ...
        FROM ( 
            SELECT
                ...
                T1.top_craftsman_id AS top_craftsman_id,
...
dwh_delta_update_result AS ( -- Расчёт данных для обновления существующих записей
    SELECT 
        T4.customer_id AS customer_id,
        T4.customer_name AS customer_name,
        T4.customer_address AS customer_address,
...
INSERT INTO dwh.customer_report_datamart (customer_id, customer_name, customer_address, customer_birthday, customer_email, customer_spent, platform_earnings, order_count, avg_order_price, median_order_completion_time, top_product_category, top_craftsman_id, count_order_created, count_order_in_progress, count_order_delivery, count_order_done, count_order_not_done, report_period)
